...
function displayStudents(students, courses) {
  ...
  students.forEach(s => {
    ...
    li.innerHTML = `<strong>${s.name}</strong> (${s.age})<br>
      <small>Enrolled in: ${course ? course.title : "No course assigned"}</small>`
    if (s.age > 21) {
...
function displayInstructors(instructors) {
  ...
  instructors.forEach(i => {
    const li = document.createElement('li')
    li.innerHTML = `<strong>${i.name}</strong><br><small>Teaches: ${i.subject}</small>`
    ul.appendChild(li)
  })
...
function displayRelationships(students, courses, instructors) {
  ...
  students.forEach(s => {
    const course = courses.find(c => c.id === s.courseId)
    const instructor = instructors.find(i => i.id === (course ? course.instructorId : null))
    const li = document.createElement('li')
